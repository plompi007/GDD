...
pub fn spawn_smoke_test_scene(
    ...
    spawn_part(&mut commands, &mut meshes, &mut materials, floor, Vec2::new(0.0, -300.0), 0.0);

    let plank = parts
        .get("plank_wood")
        .expect("plank_wood must be a registered P0 part");
    spawn_part(&mut commands, &mut meshes, &mut materials, plank, Vec2::new(0.0, -120.0), -20.0);
    ...
    let ball_entity = spawn_part(&mut commands, &mut meshes, &mut materials, ball, Vec2::new(-45.0, 60.0), 0.0);
