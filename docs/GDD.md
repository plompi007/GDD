...
#### 3.9.9 סטטוס מימוש בפועל (M6.5, נכתב אחרי סבב "Juice" ראשון)

**הרקע לסבב הזה:** אחרי שהתברר שאמנות DALL·E-generated לא מגיעה לרמת "משחק שיוצא ב-2026" (ראה הדיון בסטטוס האמנות למעלה), ההחלטה הייתה לעבור מ"אמנות טובה יותר" ל"תנועה/Juice טובה" — משחקי פיזיקה קטנים בדרך כלל *נראים* טוב בזכות תגובתיות ופידבק חזותי, לא בזכות פרטי-אמנות. הסבב הזה ממש חלק אמיתי מ-§3.9 (לא הכול — ראה "מה לא נעשה" למטה), ותיקן תוך כדי עבודה **פער אמיתי וקריטי שהתגלה, לא רק חוסר-פוליש**: `RopeConnection`/`BeltConnection` (M4) מעולם לא קיבלו שום ייצוג חזותי — אפס קו, אפס sprite, שום דבר. חבל/גלגלת/רצועה בכל 60 השלבים היו **בלתי-נראים לגמרי** מאז M4, לא רק "לא מעוטרים". זה נתפס ותוקן כחלק מהעבודה הזו, לא כפריט Juice נפרד.

**מה מומש בפועל:**

- **`render_fx.rs` (חדש) — קווי אנרגיה צבעוניים לחבלים/רצועות:** `Gizmos` (immediate-mode, מצויר מחדש כל פריים מ-Transform-ים חיים — לא צריך state של reveal/אינטרפולציה כדי שחבל מתוח או רצועה מסתובבת "יראו נכון"). חבל = TENSION (ציאן `#3DDBD9`), רצועה = ROTARY (ענבר `#F2A33D`) — בדיוק טבלת 3.9.2. קו-ליבה בהיר + קו רחב/עמום מאחוריו (4 קווים מוזזים בניצב) כתחליף זול ל-bloom אמיתי. **חייב את הרנדרר האמיתי** (`Gizmos` דורש `GizmoPlugin`, חלק מ-`DefaultPlugins`, לא קיים תחת `MinimalPlugins`) — כמו `render::PartArtPlugin`, המודול הזה לא נוסף לשום אפליקציית טסט, ואומת רק חזותית: תחת Xvfb, `lvl_a07_gear_train` (רצועה) ו-`lvl_a09_cut` (חבל דרך גלגלת) שניהם הראו קו-אנרגיה זוהר, צבוע נכון, במקום שום דבר.
- **`juice.rs` (חדש) — תגובות מטבלת 3.9.6, headless-safe:** `JuiceBounce` (scale-bounce 1.0→1.08→1.0, עקומת `sin(t·π)`, 160ms) + `Flash` (עיגול מתרחב-ודועך, בצבע קטגוריית-הרכיב) על **הצבת רכיב חדש** (מ-`input.rs`'s `end_drag_system`); `on_solved` מפעיל `Flash` מסונכרן בצבע success על כל `LevelEntity` ב-`OnEnter(Solved)` (טבלת 3.9.6's "כל הרכיבים שבפתרון מהבהבים"); `FailOverlay` — כהות עולה בהדרגה על כל המסך (bevy_ui `Node`, 400ms, ~0.55 אלפא מקס') ב-`OnEnter(Failed)`, מתנקה ב-`OnEnter(Edit)`. **הכול headless-safe** (`Mesh2d`/`ColorMaterial`/`bevy_ui::Node` בלבד, בלי `AssetServer`) — בניגוד ל-`render_fx.rs`, זה **כן** נבדק אוטומטית: `tests/juice.rs` (3 טסטים חדשים) מוכיח שהצבה מייצרת ומנקה `JuiceBounce`/`Flash`, שפתרון אמיתי (`lvl_a01_first_roll`) מפעיל `Flash`, ושמעבר ל-Failed/Edit מייצר/מנקה `FailOverlay`. **153 טסטים עוברים** (150 + 3). אומת גם חזותית: קו-חבל/רצועה זוהר (למעלה), והבהוב-ניצחון ירוק מסונכרן על כל חלקי `lvl_a01_first_roll` נתפס בצילום-מסך אמיתי ברגע ה-Solved (תוך פולינג של ה-log כדי לתפוס את החלון הקצר של 600ms במדויק — הבהוב-הצבה (160ms) התברר קצר מדי לתפוס באמינות דרך latency של round-trip בין פקודות Xvfb נפרדות, אבל מאומת באמצעות הטסט האוטומטי שמפעיל בדיוק את אותו code path).

**מה לא נעשה (במפורש, לא נשמט בשקט):**

- ~~**קידוד-צבע לפי אנרגיה על `WIRE`**~~ **— נסגר בעדכון מאוחר יותר, ראה למטה.**
- **חלקיקים זורמים אמיתיים (§3.9.2's "shader של זרימת חלקיקים"):** קו סטטי/זוהר בלבד, לא חלקיקים נעים לפי עוצמת אות. לא מומש.
- **squash-stretch על שבירה/פגיעה, קונפטי בניצחון:** טבלת 3.9.6 מבקשת את שניהם — קונפטי ידרוש `rand` (מותר לפי CLAUDE.md כלל 2 בשכבת רינדור, אבל לא הוצדק כתלות חדשה לאפקט בודד בסבב הזה); squash-stretch-לפי-מסה על שבירה לא מומש.
- **Desaturate אמיתי (per-material):** `FailOverlay` הוא קירוב (overlay כהה שדוהה פנימה), לא desaturation אמיתי לכל חומר/ספרייט בסצנה — הוצהר במפורש בקוד כפשרה מכוונת, לא נסיון-שנכשל.
- **כל §3.9.3 (shader "וקטור עמוק" — תאורה, AO, parallax, bloom/vignette/chromatic-aberration אמיתיים) ו-§3.9.5 (Glass HUD עם `backdrop-filter: blur`):** עבודת shader/post-processing מותאמת אישית ש-Bevy לא נותן "בחינם" — לא הותחלה כלל בסבב הזה. `MOTION`/`RADIUS`/`SPACE`/`ENERGY_COLORS`/`PALETTE` ב-`ui/tokens.rs` נשארים מקור-האמת היחיד (§3.9.1), אך העקומות (`cubic-bezier`) עדיין לא מחוברות למערכת tween אמיתית — ה-Juice שמומש כאן משתמש בעקומת `sin` פשוטה כתחליף זול, לא בעקומות המדויקות מהטבלה.

**עדכון: קידוד-צבע לפי אנרגיה על `WIRE` נסגר — שלושת סוגי החיבור (חבל/רצועה/חוט) מציגים קו-אנרגיה זוהר עכשיו, לא רק שניים.** הבעיה המקורית הייתה אמיתית: ל-`WIRE` (ELECTRIC) אין שום ישות פיזית מתמשכת — `EnergyGraph::connect()` עובד לפי מזהי-string, לא `Entity`, אז לא היה למה לצייר קו *בין*. הפתרון: `energy_graph.rs` מקבל component קוסמטי חדש, `WireConnection { from: Entity, to: Entity }` — לא נוגע ב-`EnergyGraph` עצמו (הלוגיקה החשמלית האמיתית) בכלל, רק "זוכר" איזה שתי ישויות-חיות מחוברות, בדיוק כמו ש-`RopeConnection`/`BeltConnection` כבר עושות. נוצר גם ב-`level_load.rs` (טעינת JSON) וגם ב-`input.rs`'s connect-tool (Sandbox חי), לצד קריאת ה-`energy.connect()` הקיימת. `render_fx.rs` מקבל `draw_wires` חדש — קו ישר מרכז-למרכז (בניגוד לרצועה, אין שפת-גלגלת; בניגוד לחבל, אין גלגלות-ניתוב), בצבע ELECTRIC (`#F4E04D`, צהוב) בדיוק לפי טבלת 3.9.2. 158 טסטים עדיין עוברים (התוספת קוסמטית-בלבד, לא נוגעת בפיזיקה/בדטרמיניזם). אומת חזותית תחת Xvfb: **לא** על שלב אמיתי (המיקומים הגיאוגרפיים של switch/motor בשלבים הקיימים כמו `lvl_a06_switch`/`lvl_b04_launch_switch` יושבים הרחק מהמרכז ומחוץ למסך הנראה, בעיה גיאומטרית קיימת ולא קשורה לשינוי הזה) אלא על שלב-scratch זמני שנבנה במיוחד לבדיקה (שני החלקים ממורכזים, לא נשמר בעץ הקוד) — הראה קו צהוב זוהר בין `switch_plate` ל-`motor_electric`, תואם בדיוק לספק.
...
| M6.5 | **Visual polish pass (v1.1)** | render systems לחלקיקי אנרגיה צבועים (§3.9.2), Juice מלא (טבלת 3.9.6), haptics אנדרואיד | וידאו של שלב שנפתר "מרגיש" כמו טריילר של משחק, לא פרוטוטייפ — **⏳ חלקי, ראה סטטוס מפורט למטה** |
| **M7** | **אריזת דסקטופ + אנדרואיד + CI** | **GitHub Actions matrix: דסקטופ (`ubuntu`/`macos`/`windows-latest`, `cargo build --release` + `cargo test`) `.github/workflows/desktop.yml`, **וגם** `.github/workflows/android.yml` לבניית APK debug** | **⏳ חלקי: דסקטופ CI ✅ הושלם ומאומת ירוק בפועל בשלושת המערכות (`mcp` GitHub Actions — לא רק "קיים", בפועל `conclusion: success` בכל 3). אנדרואיד CI ✅ מאומת ירוק אחרי 4 באגים אמיתיים שנמצאו ותוקנו ברצף (פלטפורמה חסרה → אזל שטח דיסק → ניקוי-דיסק ששיבש את התקנת ה-NDK → `cargo apk` לא הצליח להבחין בין `[[bin]]` ל-`[lib] cdylib` עם אותו שם) — ה-run הירוק האחרון (`1b11fe3`) בנה, חתם והעלה ארטיפקט APK debug אמיתי בגודל 68MB. **עדיין חסר**: זה APK **debug** בלבד (לא release-signed כנדרש בקריטריון), ואין עדיין אימות בפועל על טלפון פיזי (M3.5 עדיין פתוח מהסיבה הזו) — release signing הוא M10/M11. **תוקן בדיעבד (התגלה במהלך M9):** `desktop.yml` לא העלה בעבר ארטיפקט להורדה בכלל, רק בנה+בדק — ובנוסף, הרצת הבינארי הבנוי *ישירות* (לא דרך `cargo run`) מגלה שגופנים ל-UI לא נטענים, כי `bevy_asset` נופל ל-fallback שמחפש `assets/` לצד הבינארי עצמו כש-`CARGO_MANIFEST_DIR` לא קיים (הוא קיים רק כש-Cargo מריץ את הפרוצס). שני אלה תוקנו יחד: שלב חדש מעתיק `assets/` לצד הבינארי ומעלה תיקיית `dist/` שלמה כארטיפקט לכל אחת משלוש המערכות — ראה §7.8 המלא** |
| **M8** | **תוכן** | כל 60 שלבים + P0/P1 מלאים + כל `solutions[]` golden tests עוברים ב-CI | **✅ הושלם: כל 60 השלבים (חטיבות A–D) בנויים, נבדקו כ-solvable+deterministic ב-`cargo test`, ומעודכנים ב-`level_catalog.rs`'s `ALL_LEVELS`. חטיבה A (A_FOUNDATIONS, שלבים 1–12 לפי §5.8's 12+16+16+16=60) הושלמה — 12/12 שלבים ממומשים ומאומתים (`tests/content_levels.rs` + `tests/gear_and_pulley.rs`; התיאור הקודם "9/10" כאן היה מונה שגוי/מיושן — 13–28 שמור ל-B, ולכן A חייבת להיות 1–12, לא 1–10). כולל שלוש תת-מערכות חדשות: A01 (נפילה חופשית), A02 (`springboard`), A04 (`lever_seesaw`), **A05 (`atmosphere.rs`'s `FieldSystem` חדש — שדה PNEUMATIC מלבני מ-`fan_blower`, דוחף כל גוף דינמי לפי `windFactor`/מסה הפוכה)**, **A06 (`switch_plate` חדש — IMPACT דרך `CollidingEntities` קיים, `SimSet::CollisionRouting`, ELECTRIC out ל-`EnergyGraph`)**, A07 (גלגלי שיניים+מסוע), **A08 (`atmosphere.rs`'s `ThermalSystem` חדש — `candle`/`fuse_cord`/`charge_barrel`: הצתה מבוססת-קרבה, התקדמות בעירה קבועה, פיצוץ רדיאלי + הריסת `DESTRUCTIBLE`)**, **A09 (`sim/collision_router.rs` חדש — כלל תגית גנרי SHARP↔POPPABLE, ו-`cutter_shears` חדש שחותך `RopeConnection` וקרבה)**, **A10 (`punch_arm` חדש — אימפולס קדימה על מגע חדש דרך אותו `CollisionRouting`)**. A03 קיים כשלב-פיתוח (`lvl_a03_pulley_lift`, לא לפי המספור המקורי). **A11 "Handle With Care" (`ball_glass`/FRAGILE כ-SUBJECT בפני עצמו לראשונה — עד כה FRAGILE הופיע רק כמכשול ב-B03; כאן המטרה להוכיח שגלגול עדין דרך רמפה רגילה לא חוצה את סף השבירה, מהירות שיא נמדדה ~344 יח'/שנ' מול הסף 576)**, **A12 "Clear the Air" (`spike_pin`/SHARP כ-פותר-מכוון בפני עצמו לראשונה — בלון חוסם שוקע/עף אל מסמר ונפוץ בתוך כ-30 טיקים, הרבה לפני שהכדור המתגלגל לאט מגיע לאזור, מפנה את הדלי; בניגוד ל-C05 שבו הכלל הזה משמש כאיום למנוע, כאן הוא מנוע-פתרון מכוון)**. באג אמיתי שנמצא ותוקן באגב: `PartTags` (`level_load.rs`) כלל בעבר רק תגיות per-instance מה-level JSON, לא את תגיות ה-part-type המובנות (`DESTRUCTIBLE`/`FLAMMABLE`/`POPPABLE`/`SHARP` מ-`data/parts/*.json`) — כלל SHARP↔POPPABLE ופיצוץ-הורס-DESTRUCTIBLE היו no-op שקט לפני התיקון. חטיבת B (B_COMBOS, שלבים 13–28 לפי §5.8): 16/16 שלבים ממומשים — **הושלמה** ומאומתים (`tests/content_levels_b.rs`): B01 "Cut Loose", B02 "Relay", B03 "Break Through" (כלל תגית FRAGILE חדש), B04 "Fling the Trigger", B05 "Windswept" (ראו למעלה), **B06 "Three in a Row" (שרשרת 3 גלגלים מותאמים — motor→gear1→gear2→gear3→מסוע. באג אמיתי שנמצא: כל צימוד גלגלים נוסף הופך את כיוון ההנעה, כך ששרשרת של 3 גלגלים צריכה כיוון מנוע הפוך משרשרת של 2 כדי שהמסוע יצא לאותו כיוון בפועל — נתפס כי הכדור פשוט נסע לכיוון ההפוך ונפל מהעולם)**, **B07 "Bounce Relay" (`springboard` (A02) משגר כדור-הדק בקשת אווירית אל `switch_plate` — השימוש הראשון בשילוב הזה; הלוח ממוקם לא לפי ניחוש אלא לפי מסלול הקשת שנמדד בפועל דרך רתמת ה-debug — לחיצה מלאה במגע אווירי חד-פעמי מספיקה כי המצב הוא toggle, ואז מנוע/גלגלים/מסוע (B01) מעבירים כדור-נושא נפרד לדלי; נפתר בניסיון התכנון השני אחרי שהניסיון הראשון מיקם את הלוח גבוה מדי מעל קודקוד הקשת בפועל)**, **B08 "Cut and Catapult" (`lever_seesaw` (A04, וכבר גם ב-B04) הפעם משולב עם חבל+גלגלת+`cutter_shears` (A09) — שילוב חדש, לא ה-lever הראשון בחטיבה B: משקולת תלויה מוחזקת ע"י חבל מתוח דרך גלגלת עד עוגן שוכב על הקרקע; חיתוך החבל משחרר אותה בדיוק כמו נפילת המשקולת המקורית של A04 — משיגה תזמון שחרור מיידי (elec_in של החותך מחובר ישירות ל-`outlet_power`, ללא כדור-הדק מפיל) כדי לא לאבד את "מרוץ הזמן" מול הנטייה הטבעית של הכדור-נושא להחליק מהמוט לפני שהמשקולת מגיעה. שני ניסיונות עיצוב אמיתיים שנכשלו ותועדו כלקח: (1) רוח (`fan_blower`) שאמורה לסטות ארגז נופל כבד מעל מרחק גדול לכיוון מוט — נכשל כי נוסחת הדחיפה (`accel = power * windFactor * (1/mass) * falloff`) חלשה מכדי להזיז גוף כבד שנופל מהר על פני מאות יחידות בזמן המוגבל שהוא שוהה בתוך השדה; (2) `punch_arm` שמשגר משקולת אנכית מעלה ישירות מעל המוט — נכשל כי הדחיפה הראשונית החלשה-יחסית איפשרה למוט להתחיל להטות והכדור-נושא להחליק ממנו כמעט מיד, עוד לפני שהמשקולת המשוגרת השלימה את מעופה וחזרה ליפול. נפתר רק בגרסה השלישית, שחזרה במדויק לפיזיקת ה-drop המוכחת של A04 והוסיפה רק את מנגנון ה"החזקה עד לחיתוך")**, **B09 "Blow the Switch" (`fan_blower` דוחף כדור-הדק קל (`ball_rubber`, מסה 0.6) שמתגלגל על הקרקע (לא נופל באוויר כמו בניסיון הרוח שנכשל ב-B08) עד ללוח לחיצה — שימוש ראשון ברוח כמנגנון שמפעיל `switch_plate` במקום מגע-כבידה רגיל. עובד בזכות חשיפה ממושכת לשדה תוך כדי גלילה נתמכת-קרקע, בניגוד לניסיון הכושל הקודם של הזזת ארגז כבד שנופל באוויר על פני מרחק גדול)**, **B10 "Four in a Row" (הרחבה ישירה של לקח-הכיוון מ-B06 לשרשרת של 4 גלגלים — 3 צימודים (אי-זוגי, כמו הצימוד היחיד ב-A07) צריכים לכן את אותו כיוון מנוע כמו A07 (CCW), לא כמו B06 (2 צימודים, זוגי, CW). ניחוש-חוקיות זה אומת נכון בניסיון הראשון דרך הרצת הטסט בלבד, בלי צורך ברתמת debug)**, **B11 "Snip and Drop" (שימוש ראשון ביכולת השנייה של `cutter_shears` — פיצוץ POPPABLE בקרבה (`cut_radius`), לא רק חיתוך חבל. כדור-הדק מתגלגל במורד רמפה ונוגע בחותך (IMPACT, בדיוק כמו A09), שמפוצץ בלון-חוסם קרוב שעומד בדרכו של כדור-נושא נופל בנפרד. הכדור-נושא פשוט "יושב"/מתנדנד בעדינות מעל הבלון עד שהוא נעלם ואז ממשיך ליפול — נבדק ואומת שההתנהגות דטרמיניסטית ולא כאוטית למרות המגע הראשוני עם הבלון)**, **B12 "Blast the Glass" (שילוב חדש: פיצוץ חבית (A08) עם כלל FRAGILE (A11/B03) — הפעם לא דרך `punch_arm`, אלא דרך הדחיפה הרדיאלית הגנרית שהפיצוץ מפעיל על **כל** גוף דינמי בטווח (לא רק הריסת DESTRUCTIBLE). כדור זכוכית יושב 40 יח' מהחבית, מקבל דחיפה ~660 יח'/שנ' (מעל סף 576) ומתנפץ כשהוא נוחת על הקרקע כמה טיקים אחר כך — בעוד באותו פיצוץ ממש גם הקיר הרגיל (DESTRUCTIBLE) נהרס כרגיל, שני אפקטים מפיצוץ אחד. ניסיון עיצוב ראשון שנכשל ותועד כלקח: קרש קפיצה (B07) שאמור לשגר כדור ישירות אל הזכוכית — נכשל כי מהירות השיא לאורך כל קשת הקפיצה בפועל (עד ~670 בנפילה החופשית לפני הקפיצה עצמה, אך רק ~200-500 לאורך הקשת שאחריה) מעולם לא חפפה גם את המהירות הנדרשה וגם את מיקום הזכוכית בו-זמנית — שיטת הפיצוץ הרדיאלי, עם נוסחה מדויקת וניתנת לחישוב מראש (`velocity = blast_power * (1 - dist/blast_radius)`), הייתה אמינה בהרבה)**, **B13 "Chain Reaction" (שרשרת קצרה: כדור-הדק מתגלגל ללוח1→לוח1 מפעיל מאוורר (A05)→הרוח מגלגלת כדור-הדק קל שני על הקרקע ללוח2 (B09)→לוח2 מפעיל מנוע/גלגלים/מסוע (B01). גרסת-B קצרה של אותה תבנית ש-D04 השתמשה בה בתוך שרשרת ארוכה יותר. שיעור עיצוב אמיתי שנתפס תוך כדי: ניסיון ראשון ("Push the Crate") ניסה להרחיב את לקח-הרוח של B09 לגוף כבד (`crate_wood`, מסה 3.0 לעומת מסת ה-`ball_rubber` 0.6) — נכשל **לחלוטין**, אפילו בעוצמת מאוורר 150 (פי 18.75 מהמקורי): קופסה שוכבת על הקרקע ונחה ממקום לא זזה בכלל, לא רק לאט — ממצא חדש ושונה משני הכישלונות הקודמים של B08 (לא "כוח חלש מדי לאורך מרחק", אלא נעילת-חיכוך-סטטי מוחלטת של גוף מרובע נייח, כנראה כי שטח המגע הגדול של קופסה שוכבת שונה מהותית מכדור המתגלגל שב-B09 עבד מצוין). מוחלף בתבנית הבטוחה של B09)**, **B14 "Fragile Cargo" (`ball_glass`/FRAGILE כ-SUBJECT רוכב על מסוע לראשונה — מוכיח שתנועת המסוע עדינה מספיק שלא לשבור זכוכית. באג מיקום אמיתי שנתפס ותוקן דרך רתמת ה-debug: ה-SUBJECT הוצב לפי הקואורדינטה של B01 (x=470) בלי לשים לב שרוחב המסוע במפה הזו שונה (מרכז ב-x=600, חצי-רוחב 110 → טווח [490,710]) — 470 נפל **מחוץ** לתחום המסוע לגמרי, אז הכדור פשוט נפל לצד המסוע ונח על הקרקע, מעולם לא נסע. תוקן ע"י הזזת ה-subject ל-x=520, בתוך תחום המסוע בפועל)**, **B15 "Ride to the Zone" (מסוע (B01) מספק כדור ל-REACHED_ZONE במקום CONTAINED/bin_target — לא נבדק בעבר; מוכיח ש-`goal_zone` וה-CONTAINED/REACHED_ZONE conditions שניהם קוראים בדיוק אותה בדיקת `CollidingEntities` גנרית. באג רוחב-עולם אמיתי שנתפס ותוקן: `world.width: 1200` (half-width 600) בעוד המסוע וה-goal_zone ממוקמים עד x=710-722 — הכדור-נושא חצה את חצי-הרוחב המוצהר של העולם באמצע הרכיבה על המסוע וקיבל LEFT_BOUNDS בטרם הגיע לאזור; תוקן ע"י הרחבת העולם ל-1800)**, **B16 "Rise and Shine" (בלון עולה ופוגע ב-`switch_plate` ממוקם מעליו מלמטה — הוכחה ש-IMPACT contact עובד מכל כיוון, לא רק נפילה/גלגול על הלוח. נפתר בניסיון הראשון, Solved ב-tick 540)** — **חטיבה B הושלמה: 16/16**. תיקון תחזוקה אמיתי שנמצא באגב (במהלך B07): `src/level_catalog.rs`'s `ALL_LEVELS` (רשימת השלבים שהמשחק בפועל מאפשר לשחקן להגיע אליהם, דסקטופ ואנדרואיד כאחד) לא כלל את D01 כלל — נבנה ונבדק בהצלחה אך היה בלתי-נגיש בפועל דרך המשחק עד לתיקון הזה. חטיבת C (C_TIMING, שלבים 29–44): 16/16 שלבים — **הושלמה** — C01 "Slow Fuse" (כלל THERMAL×TENSION חדש ב-`atmosphere.rs`/`rope_network.rs`: חבל בטווח מקור חום נשרף ונחתך, בדיוק כמו §1.4.1 המתועד אך מעולם לא מומש. גם באג פיזיקה אמיתי: כדור שנופל אנכית לחלוטין ונוחת על קרקע שטוחה עם מהירות אופקית אפס יכול לפגוע בכיוון-חיכוך ניוון (degenerate) בפותר הפיזיקה ולקבל בעיטה אופקית בלתי-צפויה — A09 נחת נקי במקרה, C01 לא; תוקן ע"י מלכודת-תפיסה שלא תלויה במזל הנחיתה), C02 "Late Arrival" (קרש קפיצה (A02) שולח כדור למרוץ מול פתיל שכבר דלוק — שילוב תזמון + קפיצה ראשון). **C03 "Clear the Way" (כלל PNEUMATIC×THERMAL חדש ב-`atmosphere.rs`'s `wind_field_system` — רוח בעוצמה ≥2 מכבה נר לצמיתות, בדיוק כמו §1.4.1 המתועד אך מעולם לא מומש. אותו מאוורר גם דוחף בלון-SUBJECT מעבר למקום שבו עמד הנר; ה-fail condition `SUBJECT_DESTROYED` נשאר בשלב במכוון — מוכיח שהנר באמת כבה ולא רק שהבלון "הצליח לחמוק", כי לו הנר נשאר דלוק, ה-FLAMMABLE tag rule הקיים כבר היה מפוצץ את הבלון)**, **C04 "Just in Time" (אותו מנגנון PNEUMATIC×THERMAL כמו C03, אך הפעם מגודר מאחורי `switch_plate` — כדור-הדק נופל על הלוח ורק אז המאוורר קם לחיים, כך שיש עיכוב אמיתי לפני שהרוח מגיעה. שני באגים גיאומטריים אמיתיים שנתפסו ותוקנו דרך רתמת ה-debug בטרם ה-commit: (1) `fan_blower` הוא גוף פיזי מוצק (לא sensor) בדיוק כמו `charge_barrel`/`cutter_shears` בעבר — בלון שהוצב חופף לגוף המאוורר נתקע פיזית על הפינה שלו במקום לעלות/לנוע חופשי; (2) שדה הרוח פועל רק לכיוון ה-+x המקומי של המאוורר (`along < 0.0` נפסל ב-`in_fan_field`) — מיקום ראשוני של הבלון "מאחורי" המאוורר הפך את הבדיקה לשקרית תמיד. גם התגלה שהבלון הבועט (buoyant climb) ממשיך לעלות אנכית הרבה אחרי שהוא חוצה את טווח ה-x של אזור המטרה, כך שמיקום goal_zone לפי הגובה הסטטי המקורי (כמו ב-C03) פספס אותו לגמרי — תוקן ע"י מדידת המסלול בפועל וקביעת גובה ה-goal_zone לפי הגובה שבו הבלון חוצה את ה-x הנכון, לא לפי ניחוש)**, **C05 "Needle Point" (שימוש ראשון בפועל ב-`spike_pin`/SHARP בתוכן — לא רק בטסט יחידה. בלון עולה אנכית ישר לתוך מסמר קבוע אלא אם המאוורר (מגודר מאחורי `switch_plate`, אותו מנגנון כמו C04) מספיק לדחוף אותו הצידה לפני שהוא מגיע לגובה המסמר; אם המאוורר לא היה מגיע בזמן, הבלון היה פוגע ונפוץ (כלל SHARP↔POPPABLE הגנרי מ-A09). נבנה ונאומת בניסיון הראשון בזכות שימוש חוזר ישיר בגיאומטריית ה-fan/switch המדויקת שכבר הוכחה ב-C04)**, **C06 "Twin Candles" (שני מופעים בלתי-תלויים לגמרי של פתיל/חבית/קיר (A08) בו-זמנית באותו שלב — בדיקה טכנית שהמנוע מטפל נכון במספר `ThermalEmitter`/`FuseCord`/`ChargeBarrel` מקבילים בלי הפרעה הדדית (רדיוס הפיצוץ של חבית1 מגיע לטווח נר2 אך לא פוגע בו כי אין לו תגית DESTRUCTIBLE, ורדיוס הפיצוץ קטן מהמרחק לקיר2/חבית2). שני הקירות נהרסים כמעט בו-זמנית סביב tick 270, הרבה לפני שהכדור המתגלגל לאט מגיע לאף אחד מהם — נבנה ונפתר בניסיון הראשון)**, **C07 "Race the Belt" (מסוע (B01, מופעל ישירות מ-outlet) נושא את ה-SUBJECT בקצב קבוע וצפוי, במקום קשת קפיצה לא-צפויה (כמו C02) — שילוב חדש מבחינה מבנית עם פתיל/קיר (A08). הכדור נתקל לרגע בגוף המוצק של הנר תוך כדי המסע (לא באג — הנר כבר סיים את תפקידו וגמר לבער, אין השפעה שלילית) אך ממשיך ומגיע לדלי בבטחה, הרבה אחרי שהקיר כבר נהרס (tick270 מול tick690+ הגעה בפועל). נבנה ונפתר בניסיון הראשון)**, **C08 "Burn and Deliver" (חבל/גלגלת/פתיל שורף-חבל (C01) לראשונה משולב עם מנוע/גלגלים/מסוע (B01) — המשקולת שנופלת אחרי החיתוך נוחתת על לוח שמפעיל את המסוע, במקום ליפול ישירות לדלי כמו ב-C01. ניסיון ראשון ניסה לשלב את אותו מנגנון חבל/פתיל עם מאוורר+בלון (כמו C04/C05) — נכשל כי החיתוך+נפילה לוקחים ~390 טיקים, וזמן זה מספיק לבלון לעלות גבוה מדי מעבר לטווח האנכי הקבוע של שדה המאוורר (half_width 150) לפני שהחשמל בכלל מגיע; בניגוד ל-C04/C05 שם הלוח מופעל תוך ~180-255 טיקים בלבד ע"י כדור-הדק שנופל ישירות. לקח: שילוב rope/pulley/fuse (שתמיד לוקח כמה מאות טיקים) עם "מרוץ נגד בלון עולה" לא מתאים — בלון תמיד יספיק לעלות מעבר לכל טווח מאוורר קבוע לפני שהעיכוב הזה מסתיים; הוחלף במסוע שאין לו "טווח" זמן-רגיש כזה)**, **C09 "Punch and Beat the Flame" (אותו מרוץ מול פתיל-דלוק-מראש כמו C02, אך עם `punch_arm` (A10) כמנגנון שיגור מדויק וקבוע במקום קרש קפיצה לא-צפוי — וריאציה תוכנית על אותו רעיון עם מכניקה אמינה יותר. באג גיאומטרי אמיתי שנתפס ותוקן: ניסיון ראשון הוסיף שני קירות-תפיסה משני צידי הדלי (בהשראת התבנית המוצלחת מ-C01) — אבל C01's הכדור נוחת כמעט-אנכית מלמעלה, בעוד כאן הכדור מתגלגל אופקית במהירות מהכיוון השני; קיר-התפיסה השמאלי חסם את הכניסה לדלי לגמרי במקום לתפוס אותו, והכדור פגע בו וניתר אחורה עד עצירה מוחלטת הרחק מהדלי. תוקן ע"י הסרת קיר-התפיסה השמאלי והשארת עוצר יחיד מימין לדלי בלבד (בדיוק כמו A10/D01) — לקח: כיוון ההגעה קובע אם צריך "משפך" דו-צדדי או עוצר חד-צדדי)**, **C10 "No Time to Spare" (שימוש ראשון ב-timeLimitSec **צמוד באמת** כמכניקת C_TIMING בפני עצמה — לא רשת ביטחון נדיבה (20-35 שניות כמו כל שלב קודם) אלא מגבלה פעילה: גרסה מכווצת-מרחק של שרשרת לוח/רוח/לוח/מסוע (B13) שנפתרת ב-tick 880, עם timeLimitSec=17 (1020 טיקים) — כ-16% מרווח בלבד. ניסיון ראשון עם 15 שניות (900 טיקים) התברר צמוד מדי (מרווח של 20 טיקים בלבד, ~0.33 שנ') והוגדל ל-17 למרווח בטוח-אך-עדיין-הדוק)**, **C11 "Two Ways to Cut" (אותו חבל שמוחזק תלוי (C01) מאוים משתי מכניקות עצמאיות בו-זמנית — פתיל שורף (C01) וגם `cutter_shears` שמופעל במגע ע"י כדור-הדק נפרד — כל אחת לבד הייתה מספיקה, זה רק שאלה מי מגיע קודם. נפתר ב-tick 460, ה-cutter הגיע ראשון (נפילה קצרה מ-y=260 מהירה בהרבה מהתקדמות האש מהנר ~150 יח' משם). מדגים "נתיבים גמישים" — אותה תוצאה פיזית דרך שני מנגנונים שונים לגמרי)**, **C12 "Wind-Up" (שילוב חדש: פתיל/פיצוץ/קיר (A08) → לוח (A06) → שרשרת 3 גלגלים (B06, כיוון CW) → מסוע (B01) — כל השלבים כבר הוכחו בנפרד, כאן מורכבים יחד לראשונה. נבנה ונפתר בניסיון הראשון, Solved ב-tick 840)**, **C13 "Hurry, Gently" (FRAGILE (A11) תחת לחץ זמן צמוד לראשונה — אותו גלגול עדין מ-A11 (Solved ב-tick~690), אך עם timeLimitSec=13 (780 טיקים, ~90 טיקים מרווח) — מוכיח ש"מהר" ו"בעדינות" יכולים להיות בשני צירי אילוץ שונים בו-זמנית)**, **C14 "Slow Release" (כלל THERMAL×TENSION (C01/C08) הפעם משוחרר לא משקולת אלא ל-`balloon_lift` (A05/B16) שעצמו ה-SUBJECT — חבל מותח בלון כלפי מטה עד עוגן קבוע בקרקע; הפתיל בוער עד קרבה לעוגן, שורף את החבל ומשחרר את הבלון לעלות חופשי אל `goal_zone` (REACHED_ZONE, לא CONTAINED). ניסיון עיצוב ראשון שנכשל ותועד כלקח: אותו מנגנון שחרור-חבל נוסה קודם דרך `cutter_shears` חשמלי (כמו D02/D03) עם כדור-הדק נופל — אך `cutter_shears` הוא sensor (לא גוף מוצק) והכדור-הדק פשוט חלף דרכו והמשיך ליפול הלאה על `lever_seesaw` שהיה ממוקם באותו איזור מהניסיון המקורי (שיגור-קטפולטה כמו B08), מה שגרם לתזוזה בלתי-רצויה של המוט לפני שהמשקולת המיועדת בכלל הגיעה; יתרה מכך, כל עיכוב משמעותי בשחרור המשקולת גרם למוט (שכבר נשען רק על משקל ה-subject בצד אחד, בלי דבר שמאזן בצד השני) להטות את עצמו והכדור להחליק ממנו **לפני** שהמשקולת המשוחררת בכלל מגיעה — לקח: שילוב lever/seesaw עם עיכוב תזמון ארוך (לא כמו B08's שחרור כמעט-מיידי) לא עמיד, כי חוסר-האיזון ההתחלתי גדל ככל שהזמן עובר בלי קשר למתי המשקולת בפועל נופלת. הוחלף לגמרי בתבנית חבל-בלון/פתיל שאין לה בעיית-איזון כזו. timeLimitSec=6 (720 טיקים) מול Solved בפועל ב-tick~410 — מרווח הדוק אך אמיתי, עקבי עם C10/C13)**, **C15 "Careful Aim" (וריאציה על C09: אותה גיאומטריית פתיל/פיצוץ/קיר+`punch_arm` בדיוק, אך ה-SUBJECT הוא `ball_glass`/FRAGILE במקום `ball_wood` — משלב לראשונה את שני צירי-הלחץ של C09 (זמן) ו-C13/A11 (שבריריות) **באותו** שלב: העוצמה חייבת להספיק כדי לחצות את הפער לפני שהחומה חוזרת ונסגרת מבחינת התזמון, אך להישאר עדינה מספיק (power=70, הרבה מתחת לסף השבירה 576) כדי שהזכוכית לא תתנפץ בנחיתה/בפגיעה בעוצר. נבנה ונפתר בניסיון הראשון (Solved ב-tick 838); timeLimitSec כוונן ל-9 שניות (1080 טיקים, ~22% מרווח) אחרי מדידת זמן הפתרון בפועל דרך רתמת ה-debug)**, **C16 "One Careful Run" (שלב-סיום החטיבה: מרכיב מחדש את תבנית פתיל/פיצוץ/קיר (A08) → לוח (A06) → שרשרת 3 גלגלים (B06/C12, כיוון CW) → מסוע (B01), אך עם SUBJECT מסוג `ball_glass`/FRAGILE הרוכב על המסוע במקום `ball_wood` — משלב את כל שרשרת-המכניקות הארוכה של C12 עם דרישת-השבריריות של A11/C13/C15 יחד, כסיכום ל-C_TIMING. שיעור עיצוב אמיתי שנתפס ונדחה עוד לפני ה-commit: ניסיון ראשון ניסה לגרום לאותו כדור אחד לשמש גם כ-trigger (מתגלגל דרך פער הקיר ולוחץ את הלוח) וגם כ-SUBJECT (ממשיך ורוכב על המסוע) — נכשל כי כל מסלולי לוח/מנוע/גלגלים/מסוע הקיימים בחטיבה הזו (B01 ואילך) תמיד הציבו את ה-SUBJECT ישירות על גבי המסוע המוגבה (y≈-50) מההתחלה, לא ציפו לכדור שמגיע מגובה-הקרקע (y≈-272) בגלגול; כדור בודד שמנסה למלא את שני התפקידים פשוט ממשיך לגלגל על פני הקרקע מתחת למסוע לנצח, בלי דרך לטפס לגובהו. תוקן ע"י חזרה לתבנית המוכחת של שני כדורים נפרדים (trigger מתכת שמפעיל את הלוח בקרקע, SUBJECT זכוכית ממוקם ישירות על המסוע כמו בכל שלב קודם) — Solved ב-tick 748 בניסיון השני; timeLimitSec=8 (960 טיקים, ~28% מרווח). **חטיבת C הושלמה: 16/16**)**. חטיבת D (D_MASTER, שלבים 45–60): 16/16 שלבים — **הושלמה** — **D01 "Chain of Command" (שרשרת ארוכה: פתיל→פיצוץ הורס קיר (A08)→כדור ממשיך להתגלגל ולוחץ לוח (A06)→מנוע/גלגלים/מסוע (B01) מעביר כדור-נושא נפרד לדלי — 4 מכניקות ברצף. שיעור עיצוב אמיתי: ניסיון ראשון השתמש בדחיפת הפיצוץ (blast impulse) כדי לשגר כדור-פסולת על פני קרקע פתוחה אל לוח לחיצה מרוחק — נכשל שוב ושוב כי (א) משקולת-נגד שנחה ליד הלוח, מחוברת דרך אותו חבל/גלגלת שמחזיקה את ה-subject באוויר, "זחלה" לאט לאורך הקרקע לכיוון הלוח בגלל מתיחות החבל (התנהגות פיזיקלית אמיתית, לא באג) והגיעה ללוח לפני הפסולת המיועדת, ו-(ב) נקודת הנחיתה של הפסולת המשוגרת הייתה בלתי-צפויה וממשיכה להתכנס לאזור קרוב לאותה משקולת-זוחלת גם אחרי כיוונון עוצמת הפיצוץ. במקום להמשיך להיאבק בחישוב מסלול לא-צפוי, השלב נבנה מחדש כולו מתבניות-משנה שכבר הוכחו (הריסת קיר ע"י פתיל מ-A08, לוח-לחיצה במגע מ-A06, בלוק מנוע/גלגלים/מסוע סטנדרטי) — ונפתר בניסיון הראשון)**, **D02 "Signal Chain" (שרשרת של 5 מכניקות: פתיל→פיצוץ הורס קיר (A08)→כדור מגיע ללוח1 (A06)→לוח1 מפעיל **חשמלית** את `cutter_shears` (לא במגע — לראשונה ברמת ה-D)→חיתוך החבל משחרר משקולת תלויה שנופלת בדיוק מעל לוח2→לוח2 מפעיל מנוע/גלגלים/מסוע (B01) שמעביר כדור-נושא נפרד לדלי. באג אמיתי שנתפס ותוקן לפני ה-commit: אותה "זחילת משקולת-נגד לאורך הקרקע בגלל מתיחות חבל" שתועדה כשיעור עיצוב ב-D01 חזרה כאן בצורה מסוכנת יותר — משקולת-הנגד (שיושבת ליד הגלגלת, לא המשקולת התלויה) זחלה במקרה בדיוק עד למיקום לוח2 ולחצה אותו **בעצמה**, עוד לפני שהכדור-הדק בכלל הגיע ללוח1, מה שגרם לכל 4 המכניקות הראשונות (פתיל/פיצוץ/לוח1/חיתוך) להיות בלתי-רלוונטיות לחלוטין ולשלב "להיפתר" ב-tick 640 דרך קיצור-דרך לא-מכוון — התגלה רק כי הבדיקה בדקה גם את מצב ה-switch/rope בפועל ולא רק "האם הכדור בדלי". תוקן ע"י קיר-עצירה (`wall_brick`) בין משקולת-הנגד לגלגלת, בדיוק כמו התיקון שכבר תועד ב-D01 — לאחר התיקון השלב אכן פותר את חמשת המכניקות ברצף הנכון (Solved ב-tick 1160). לקח לתוכן עתידי: כל שילוב future של rope+pulley+counterweight חייב לכלול קיר-עצירה מונע-זחילה כברירת מחדל, לא רק כתיקון תגובתי)**, **D03 "Bounce and Cut" (שרשרת של 4 מכניקות: קרש קפיצה (B07) משגר כדור-הדק בקשת אווירית אל לוח1→לוח1 מפעיל חשמלית את `cutter_shears` (בדיוק כמו D02)→חיתוך החבל משחרר משקולת תלויה שנופלת על לוח2→לוח2 מפעיל מנוע/גלגלים/מסוע (B01). הפעם קיר-העצירה מונע-הזחילה (הלקח מ-D02) נכלל **מראש**, לא כתיקון אחרי כישלון — ואומת ישירות דרך רתמת ה-debug שהמשקולת-נגד באמת נעצרת סביב x=420 הרבה לפני שלוח2 מופעל (ב-tick 400) ע"י המשקולת התלויה שנופלת, לא ע"י זחילה. נבנה ונפתר בניסיון הראשון (Solved ב-tick 680) בזכות שימוש חוזר מדויק בגיאומטריה המוכחת של B07 (קרש+לוח) ו-D02 (חבל+גלגלת+חותך+לוח+מנוע))**, **D04 "Wind and Fire" (שרשרת של 6 מכניקות — הראשונה בחטיבה D שמשלבת רוח: פתיל→פיצוץ הורס קיר (A08)→כדור מגיע ללוח1 (A06)→לוח1 מפעיל מאוורר (A05)→הרוח מגלגלת כדור-הדק קל שני על הקרקע ללוח2 (B09)→לוח2 מפעיל מנוע/גלגלים/מסוע (B01) שמעביר כדור-נושא נפרד לדלי. נבנה ונפתר בניסיון הראשון (Solved ב-tick 1520) בזכות הרכבה ישירה משלושה תבני-משנה מוכחים בנפרד — A08 (פתיל/קיר), A06 (לוח-מגע), ו-B09 (רוח מגלגלת כדור ללוח))**, **D05 "Fan the Flame" (שרשרת של 7-8 מכניקות — המשך-הרחבה ישיר של D04: פתיל→פיצוץ הורס קיר (A08)→לוח1→מאוורר (A05)→כדור-הדק שני מתגלגל ללוח2 (B09/D04) — אך במקום שלוח2 יפעיל ישירות מנוע כמו ב-D04, הוא מפעיל חשמלית `cutter_shears` (D02/D03) שחותך חבל/גלגלת עם קיר-עצירה מונע-זחילה (הלקח מ-D02, נכלל מראש), המשקולת שנופלת נוחתת על לוח3 שרק אז מפעיל מנוע/גלגלים/מסוע (B01) עם SUBJECT מסוג `ball_glass`/FRAGILE (A11/C13/C15/C16). כל קטע נבנה מהרכבה ישירה של שני תבני-משנה מוכחים שכבר עבדו יחד בעבר (D04's front-half + D02/D03's back-half בשלמותו, כולל מרחקי ה-rope/maxLength המדויקים שנשארו זהים כי כל הגיאומטריה רק הוזזה במקביל). באג רוחב-עולם אמיתי שנתפס ותוקן דרך רתמת ה-debug בניסיון הראשון: `world.width: 2600` (half-width 1300) בעוד ה-SUBJECT ממשיך לנוע על המסוע מעבר לכך (עד x≈1301+) לפני שהוא מגיע לדלי בפועל — Failed ב-LEFT_BOUNDS ממש לפני הסוף; תוקן ע"י הרחבת העולם ל-3000. אחרי התיקון: Solved ב-tick 1692, timeLimitSec=25 (3000 טיקים, ~77% מרווח, עקבי עם הסגנון הנדיב של D01–D04))**, **D06 "Bounce and Blow" (שילוב חדש: הפתיחה של D03 (קרש קפיצה→לוח1) עם הסיום של D04 (מאוורר→כדור-הדק שני מתגלגל ללוח2→מנוע/גלגלים/מסוע) — מוכיח ש-`switch_plate` לא אכפת לו איך הכדור מגיע (קפיצה או גלגול לאורך שרשרת פתיל) כל עוד יש מגע. נבנה ונפתר בניסיון הראשון (Solved ב-tick 818) בזכות שימוש חוזר מדויק בשתי גיאומטריות מוכחות בנפרד; timeLimitSec=15 (1800 טיקים, ~120% מרווח))**, **D07 "Gentle Command" (וריאציה על D01: אותה שרשרת פתיל/קיר/לוח/מנוע/גלגלים/מסוע (A08/A06/B01) בדיוק, אך ה-SUBJECT הוא `ball_glass`/FRAGILE (A11/C13/C15/C16/D05) וה-win condition הוא REACHED_ZONE/`goal_zone` בלי קיר-עצירה (בדיוק כמו B15) במקום CONTAINED/bin — מוכיח שאין צורך בקיר בולם כדי לזכות, וש-FRAGILE שורד מסע מלא דרך שרשרת ארוכה כשהמהירויות נשארות עדינות. נבנה ונפתר בניסיון הראשון (Solved ב-tick 881); timeLimitSec=14 (1680 טיקים, ~90% מרווח))**, **D08 "Punch the Line" (`punch_arm` (A10) משולב לראשונה בחטיבה D — במקום פתיל/קרש קפיצה, כדור-הדק שמתגלגל מרמפה נפגע ע"י אגרוף שמשגר אותו אל לוח1, שמפעיל חשמלית את `cutter_shears` (D02/D03 back-half בשלמותו, כולל קיר-עצירה מונע-זחילה). באג מיקום אמיתי שנתפס ותוקן דרך רתמת ה-debug בניסיון הראשון: לוח1 הוצב תחילה ב-x=300, רק 40 יח' מהמשקולת-הנגד (`ball_lead`, רדיוס 14) שיושבת ב-x=340 — חצי-רוחב לוח (32) + רדיוס (14) = 46 > 40, כך שהם חפפו כבר ברגע ה-spawn, ולוח1 נדלק ב-tick~50, הרבה לפני שכדור-הדק-שנשלח בכלל הגיע (חיתוך-מוקדם-לגמרי-שקט, בדיוק סוג הבאג שתועד כלקח ב-D02, הפעם לא מזחילה אלא מחפיפת-spawn ישירה). תוקן ע"י הרחקת לוח1 ל-x=250 (מרחק 90 יח', בטוח). אחרי התיקון: Solved ב-tick 885, לוח1 נדלק רק כשכדור-הדק-שנשלח מגיע בפועל; timeLimitSec=12 (1440 טיקים, ~63% מרווח))**, **D09 "High Release" (מנגנון שריפת-חבל-משחרר-בלון (C14) משולב לראשונה בחטיבה D — הפעם הבלון עצמו אינו ה-SUBJECT אלא רק מנגנון-הפעלה: הוא עולה ופוגע בלוח קבוע מעליו (בדיוק כמו B16), שמפעיל מנוע/גלגלים/מסוע (B01) המעביר SUBJECT נפרד לדלי. נבנה ונפתר בניסיון הראשון (Solved ב-tick 717) בזכות שימוש חוזר מדויק בגיאומטריית ה-fuse/anchor/rope המוכחת של C14 ללא שינוי; timeLimitSec=10 (1200 טיקים, ~67% מרווח))**, **D10 "Twin Flame Relay" (תבנית שני-הנרות (C06) מובאת לראשונה לחטיבה D, עם סיום לוח/מנוע/גלגלים/מסוע (B01) ו-SUBJECT מסוג `ball_glass`/FRAGILE במקום להגיע ישירות לדלי. כדור-הדק בודד מתגלגל דרך שני הפערים (אחרי ששני הנרות/חביות מסירים את שני הקירות) ומגיע ללוח, שמפעיל את המסוע עבור ה-SUBJECT הנפרד. נבנה ונפתר בניסיון הראשון (Solved ב-tick 1299) בלי אף באג; timeLimitSec=16 (1920 טיקים, ~48% מרווח))**, **D11 "Two Cuts, One Belt" (תבנית שתי-דרכים-לחתוך (C11) מובאת לראשונה לחטיבה D — אך במקום שהכדור המשוחרר ייפול ישירות לדלי (כמו ב-C11), הוא נופל על לוח שמפעיל מנוע/גלגלים/מסוע (B01) עבור SUBJECT נפרד. נבנה ונפתר בניסיון הראשון (Solved ב-tick 628) בזכות שימוש חוזר מדויק בגיאומטריית ה-rope/pulley/cutter/fuse המוכחת של C11; timeLimitSec=9 (1080 טיקים, ~72% מרווח))**, **D12 "Clear and Carry" (תבנית חוסם-בלון/מסמר (A12) מובאת לראשונה לחטיבה D, עם סיום לוח/מנוע/גלגלים/מסוע (B01) ו-SUBJECT מסוג `ball_glass`/FRAGILE במקום נחיתה ישירה בדלי. באג חפיפת-spawn אמיתי שנתפס ותוקן דרך רתמת ה-debug בניסיון הראשון: לוח הועמד תחילה ממש מתחת לבלון-החוסם (זהה ב-x, רק 10 יח' למטה) — רדיוס הבלון (20) חורג מעבר לחצי-גובה הלוח (6) כבר ב-spawn, כך שהלוח נדלק מיד ב-tick~0 בלי קשר לכדור-הדק בכלל (אותה מחלקת-באג בדיוק כמו D08, הפעם מחפיפה אנכית ולא אופקית) — השלב 'נפתר' ב-tick 296 בלבד, מהר מדי להיות אמיתי. תוקן ע"י הזזת הלוח 70 יח' הצידה (x=370), הרחק מטביעת-ה-spawn של הבלון. אחרי התיקון: Solved ב-tick 839, נגרם בפועל ע"י הכדור-הדק שמגיע; timeLimitSec=12 (1440 טיקים, ~72% מרווח))**, **D13 "Wind Signal" (גיאומטריית לוח/מאוורר/נר/בלון המוכחת של C04 בשלמותה, אך במקום שהבלון עצמו יהיה ה-SUBJECT שמגיע ל-goal_zone, הוא פוגע בלוח שני (באותו מיקום המדויק שבו ה-goal_zone הישן היה) שמפעיל מנוע/גלגלים/מסוע (B01) עבור SUBJECT נפרד. נבנה ונפתר בניסיון הראשון (Solved ב-tick 747) בלי אף באג — מוכיח שגם לוח דק (64×12) תופס את הבלון באותה נקודה בדיוק שבה קופסת goal_zone רחבה יותר (64×64) תפסה אותו ב-C04; timeLimitSec=11 (1320 טיקים, ~77% מרווח))**, **D14 "Blast and Belt" (משלב את התופעה-הלוואי של B12 — פיצוץ שמנקה קיר **וגם** מנפץ כדור-זכוכית עומד-בצד (`bystander`, לא ה-SUBJECT ולא נדרש לניצחון, טהור פלוור) — עם סיום לוח/מנוע/גלגלים/מסוע (B01). כדור-הדק ממשיך להתגלגל אחרי הפיצוץ אל הלוח, בדיוק כמו D01. נבנה ונפתר בניסיון הראשון (Solved ב-tick 943) בזכות הרכבה ישירה משני תבני-משנה מוכחים ללא שינוי (B12's front-half, D01's back-half); timeLimitSec=13 (1560 טיקים, ~65% מרווח))**, **D15 "Four Gears, Gently" (שרשרת 4-גלגלים (B10, כיוון CCW — 3 צימודים, אי-זוגי) מובאת לראשונה לחטיבה D, אחרי פתיל/קיר/לוח (D01) ולפני מסוע/REACHED_ZONE (B15/D07/D12) עם SUBJECT מסוג `ball_glass`/FRAGILE. נבנה ונפתר בניסיון הראשון (Solved ב-tick 881) בזכות הרכבה ישירה משלושה תבני-משנה מוכחים — D01's front, B10's gear-chain, B15's zone-ending; timeLimitSec=13 (1560 טיקים, ~77% מרווח))**, **D16 "Grand Finale" — שלב-הסיום של M8 כולו: מרכיב את תבנית שני-הנרות (D10) עם שרשרת ה-cutter/pulley/counterweight/creep_stop/מנוע/גלגלים/מסוע (D02/D03/D08/D11/D05) — הפעם לוח-שני-הקירות מפעיל חשמלית את החותך (במקום מנוע ישירות כמו ב-D10), וטרם הגעת הכדור-נושא לדלי יש עוד שני שלבי-ביניים (חיתוך+נפילת-משקולת). שני חלקי השלב הם שילוב מדויק של D10's front-half (ללא שינוי) ו-D05's back-half (ללא שינוי, כולל אותה הזזה מדויקת של +250 יח' וה-maxLength הזהה 665, כי כל הגיאומטריה היא רק העתקה מקבילה). SUBJECT מסוג `ball_glass`/FRAGILE. נבנה ונפתר בניסיון הראשון (Solved ב-tick 1477) בלי אף באג — ההוכחה הטובה ביותר עד כה שהרכבת שני תבני-משנה **שלמים** ומוכחים בנפרד (במקום לבנות כל שלב מאפס) היא הדרך האמינה ביותר לתוכן ארוך ומורכב; timeLimitSec=20 (2400 טיקים, ~63% מרווח). **חטיבת D הושלמה: 16/16. M8 (60 שלבים) הושלם במלואו.** |
| **M9** | **Sandbox** | **✅ הושלם — פירוט מלא למטה** | ראה קריטריון קבלה למטה |
| **M10** | **Release readiness** | עמוד itch.io/Steam לדסקטופ; Data Safety form + נכסי Play Console לאנדרואיד | **⏳ כל מה שניתן להכין בקוד/תוכן הושלם (ראה "צ'קליסט שחרור" למטה) — נותרו רק חסימות שדורשות את המפתח בעצמו: מכשיר פיזי, keystore, חשבונות** |
| **M11** | **Soft launch** | דסקטופ: itch.io/GitHub Releases. אנדרואיד: internal testing track → production בהדרגה | **⏳ טרם התחיל, תלוי-M10** |

#### מפרט Sandbox (M9) — נכתב אחרי שכל 60 שלבי M8 הושלמו

**עדכון סטטוס אחרי מימוש בפועל:** כל המכניקה שהמפרט למטה מתאר **מומשה ונבדקה** — `Serialize`/round-trip (`level_file_format.rs`), מחיקה/סיבוב (`input.rs`'s `SelectedPart`/`EditorActionRequest`), כלי-חיבור חבל/מסוע/חוט (`ConnectToolRequest`/`connect_tool_system`), שמירה/טעינה אמיתית לדיסק (`save_load.rs`), ורמת Sandbox עצמה עם ארגז-חלקים בלתי-מוגבל (`levels/sandbox.json`, נגישה דרך מקש B בדסקטופ). **וגם קיבלו כפתורי bevy_ui אמיתיים** (`ui/editor_tools.rs`, שורת כפתורים מעל שורת ה-Play הקיימת: Delete/Rotate/Rope/Belt/Wire/Save/Load/Sandbox) — לא רק resources לטסטים. נבדק בשתי דרכים: `tests/ui_editor_tools.rs` (Interaction::Pressed סינתטי, בדיוק כמו ש-`tests/input.rs` כבר עושה ל-BinSlot) **וגם** הרצה אמיתית של הבינארי המקומפל תחת Xvfb עם צילום-מסך אמיתי של החלון החי, שהראה ששורת הכפתורים החדשה נמצאת במקום הנכון והאפליקציה ממשיכה לרוץ בלי קריסה. 146 טסטים עוברים. **תגלית לוואי מצילום-המסך — נמצאה ותוקנה בפועל:** גופנים (Rubik) לא נטענים כשמריצים את הבינארי הבנוי ישירות (לא דרך `cargo run`) — טקסט UI (גם הישן) הציג ריק בהרצה כזו. שורש הבעיה (מאומת בקוד המקור של `bevy_asset`): `get_base_path()` קורא את `CARGO_MANIFEST_DIR` מה-environment, שקיים רק כש-Cargo עצמו מריץ את הפרוצס (`cargo run`/`cargo test`) — לא כשמישהו מוריד ומריץ בינארי ישירות, המקרה המדויק של הפצה אמיתית (M10/M11). ב-fallback הוא הופך לתיקיית הבינארי עצמו. אומת בדיוק ע"י הרצה תחת Xvfb: בלי `assets/` לצד הבינארי — כל הטקסט (כולל שורת הכפתורים החדשה) ריק; עם `assets/` מועתק לצידו — טקסט אמיתי מוצג (כותרת "First Roll", מטרה, כל 8 תוויות הכפתורים). **תוקן:** `.github/workflows/desktop.yml` מקבל שלב חדש שמעתיק `assets/` לצד הבינארי הבנוי (`--release`) ומעלה תיקיית `dist/` שלמה כארטיפקט לכל אחת משלוש המערכות — זה בדיוק מה שגרסת-הורדה אמיתית (itch.io/GitHub Releases) תצטרך. **מה שעדיין חסר (בעדכון הזה):** נתיב-Android אמיתי ל-`saves/` (`save_load.rs`'s `default_saves_dir()` הוא desktop-only, מתועד כפער ידוע), ורשימת "My Levels"/שמות-שמירה מרובים (דורשת שדה טקסט ומסך LevelSelect שעדיין לא קיימים). **שני הפערים האלה נסגרו בסבב נפרד לאחר מכן — ראה הפסקה הבאה.**

**עדכון: שני הפערים הנותרים של M9 נסגרו — Sandbox (M9) הושלם.**

**נתיב-שמירה אמיתי לאנדרואיד, לא ניחוש.** `save_load::default_saves_dir()` היה desktop-only במפורש (יחסי ל-CWD, לא נתיב-פרטי-לאפליקציה אמיתי באנדרואיד). התיקון משתמש ב-API הציבורי האמיתי ש-`bevy_winit`'s Android backend כבר מחזיק בו לכל מטרה כזו בדיוק: `bevy::window::ANDROID_APP: OnceLock<android_activity::AndroidApp>` (מאומת בקוד המקור: `bevy_window` 0.16.1 חושף אותו תחת `#[cfg(target_os = "android")]`), ו-`AndroidApp::internal_data_path()` הוא בדיוק המקבילה האמיתית ל-`Context.getFilesDir()` — לא נתיב מנוחש, לא env-var-convention. `#[cfg(target_os = "android")]` branch נפרד ב-`save_load.rs` מחזיר `internal_data_path().join("saves")`, עם fallback ל-`"."` אם `ANDROID_APP` עדיין לא אותחל. **לא ניתן לאמת קומפילציה מקומית בסביבה הזו:** `cargo check --target aarch64-linux-android` נכשל לא בקוד שלי אלא ב-`android-activity`'s build script עצמו (דורש `aarch64-linux-android-clang++` מה-NDK כדי לקמפל את ה-C++ glue של GameActivity, שלא מותקן בסביבת הפיתוח הזו) — בדיוק אותה מגבלת-סביבה שכבר תועדה ב-M7 (התקנת NDK גרמה בעבר לבעיות דיסק אמיתיות). הקוד עצמו מוסתר תחת `#[cfg(target_os = "android")]` כך שאינו משפיע כלל על shipset הדסקטופ/הטסטים (`cargo build`/`cargo test` הרגילים לא נוגעים בו בכלל) — אימות אמיתי ימתין ל-`android.yml` CI (שכבר בונה APK דסקטופ ירוק) או להתקנה פיזית בפועל (M3.5, עדיין פתוח).

**"My Levels" — שדה שם-שמירה אמיתי + רשימת-בחירה, לא slot קבוע אחד.** `ui/editor_tools.rs` קיבל `SaveNameField` — תיבת טקסט אמיתית (bevy_ui, לא input HTML) שמקבלת הקלדה חיה דרך `EventReader<KeyboardInput>` (לא `ButtonInput<KeyCode>` הקיים, שרק אומר "האם מקש לחוץ" ולא "איזה תו הוקלד"), מסוננת בכל הקלדה לתווים בטוחים-לשם-קובץ (`is_allowed_name_char`/`save_load::sanitize_file_stem`) — **תיקון אבטחה אמיתי, לא רק פוליש:** לפני העדכון הזה שם-שמירה תמיד היה `level.id` הפנימי (בטוח, לא מהמשתמש); ברגע ששם-שמירה בא מהקלדה חופשית, קלט לא-מסונן כמו `"../../etc/passwd"` היה path-traversal write אמיתי, לא רק קלט לא-מטופל. כפתור Load כבר לא טוען slot קבוע ישירות — הוא פותח רשימת "My Levels" (`LoadMenuOpen`/`sync_load_menu`, קורא ל-`save_load::list_saved_levels` הקיים), וכל שורה בה טוענת את הקובץ שלה. פוקוס-מקלדת (`TextInputFocus`) מונע מקיצורי-הדרך של `app.rs` (Space/R/N/P/B) לפעול בזמן הקלדה — נבדק במפורש כי הקלדת "Bridge Run" מכילה בדיוק את האותיות B/R/N/P שכבר משמשות כמקשים גלובליים. **באג אמיתי שנתפס רק דרך הרצה חיה תחת Xvfb, לא ע"י הטסטים ההדסלס:** מקש הרווח מגיע ב-winit האמיתי כ-`Key::Space` (variant נפרד ב-`bevy::input::keyboard::Key`), לא כ-`Key::Character(" ")` כפי שהיה סביר להניח (ואיך שה-fake הראשוני בטסט שגה בדיוק) — הקלדת "Bridge Run" בפועל הראתה "BridgeRun" בלי רווח. תוקן ב-`save_name_keyboard_system` (`match` arm נפרד ל-`Key::Space`), ותוקן גם ב-`tests/ui_editor_tools.rs`'s `type_char` helper עצמו כך שישקף את ההתנהגות האמיתית קדימה. 158 טסטים עוברים (היו 153 — 5 חדשים: `sanitize_file_stem`'s טסט path-traversal, והרחבת `tests/ui_editor_tools.rs` מ-4 ל-8 טסטים). אומת גם חזותית תחת Xvfb: שדה השם עם highlight-כחול בפוקוס וסמן `_`, Save עם שם ריק נופל בחזרה ל-`level.id` (ההתנהגות הישנה, ללא רגרסיה), ורשימת "My Levels" האמיתית (עם `AssetServer`, לא רק resources) מציגה שורה אמיתית לכל שמירה ונטענת בלחיצה. **הערה על אמינות הבדיקה החזותית עצמה:** לחיצות עכבר תחת Xvfb ללא window manager עבדו באופן עקבי לחלוטין; הקלדת-מקלדת חיה הייתה **בלתי-אמינה** (אזהרות `XGetInputFocus returned the focused window of 1` — מגבלת-סביבה של Xvfb-ללא-WM, לא של הקוד) — אומתה בהצלחה פעם אחת בלבד (שם בדיוק נתפס הבאג של `Key::Space`), אחרי זה נכשלה שוב ושוב; שאר הזרימה (Save/Load/רשימה/טעינה) אומתה במלואה דרך עכבר-בלבד, שהיה אמין ב-100% מהניסיונות.

**תוספת גרפיקה (`render.rs`, לא M6.5 — צעד ביניים קטן, לא Prism Foundry המלא):** `body_factory.rs`'s `placeholder_color` נשאר, אבל שני שינויים אמיתיים נוספו מעליו. (1) **תוקן באג ויזואלי אמיתי:** כדורים (`ShapeSpec::Ball`) עד כה רונדרו כ-`Sprite::from_color` מרובע (בגודל תיבת-החוסם של ה-collider, לא הצורה שלו בפועל) — כדור נראה כריבוע. תוקן: `ShapeSpec::Ball` מקבל עכשיו `Mesh2d`+`MeshMaterial2d<ColorMaterial>` עם `Circle` אמיתי מ-`bevy::math::primitives` (Meshable), אותו הצבע מ-`placeholder_color` בדיוק. `ShapeSpec::Box` נשאר `Sprite` רגיל (מלבן שטוח כבר תואם לחלוטין את ה-collider שלו). (2) **`render::decorate`** — פונקציה חדשה שמוסיפה ילדים קוסמטיים-בלבד (`Mesh2d` בלי שום collider/physics component, לא משפיעים על הסימולציה) מעל 12 מתוך 29 סוגי הרכיבים: `gear_small`/`gear_large` (שיניים סביב + hub), `fan_blower` (3 כנפיים), `candle` (פתיל+שביב אש), `switch_plate` (כפתור מודגש), `motor_electric` (4 חישורים), `pulley_wheel` (hub), `cutter_shears` (X של מספריים), `charge_barrel` (2 חישוקים), `conveyor` (3 חצי-חן כיוון), `punch_arm` (חוגה קדמית), `outlet_power` (2 שנאים). שאר 17 הסוגים ממשיכים עם הצורה השטוחה המקורית ללא שינוי — לא נדרש מכל סוג להיות מעוטר כדי שזה יהיה שיפור אמיתי. אומת חזותית תחת Xvfb (בינארי-דוגמה זמני שספן את כל 12 הסוגים ברשת וצולם — לא נשאר בעץ הקוד): כל 12 הצורות נראות מובחנות זו מזו, והכדורים אכן עגולים. 146 טסטים עדיין ירוקים (כולל טסט הדטרמיניזם — הדקורציות הן ישויות-ילד קוסמטיות בלבד, לא נכנסות לפיזיקה). **מה זה לא:** לא Prism Foundry (M6.5 למעלה) — אין קידוד-צבע-לפי-אנרגיה, אין Juice/tween, אין אמנות בטמאפ/וקטור אמיתית; זו גיאומטריה פרוצדורלית ב-Rust בלבד, כי לא היה כלי ליצירת bitmap/SVG בסשן הזה. M6.5 עצמו עדיין "טרם התחיל" כמתועד למעלה.

**עדכון: אמנות אמיתית (bitmap) הוחלפה פנימה, לא רק גיאומטריה פרוצדורלית.** המשתמש ייצר אמנות אמיתית דרך DALL·E (לפי גיליון prompts שנכתב עבור כל אחד מ-29 סוגי הרכיבים, עם עוגן-סגנון משותף: "flat 2D vector game icon, minimalist, solid flat colors, thick outline, transparent background") והעביר את התוצאות. תהליך שילוב: (1) כל תמונה נחתכה מהקומפוזיציה שהוחזרה (פאנל בודד/2 פאנלים זה-לצד-זה/גריד 2×2/3 פאנלים, לפי הבקשה) דרך ImageMagick `convert -crop`; (2) הרקע הלבן הוסר לשקיפות אמיתית (אלפא, לא רק "לבן" ויזואלי) דרך floodfill מ-4 הפינות עם `-fuzz 12% -fill none`, ואז `-trim` לגזירה הדוקה לתיבת-התוכן בפועל; (3) כולן כווצו ל-256px מקסימום (`mogrify -resize 256x256>`) כדי לא לנפח את הבילד לאנדרואיד סתם. התוצאה: 27 מתוך 29 סוגי הרכיבים יש להם אמנות אמיתית תחת `assets/parts/*.png` (חסרים רק `candle` ו-`gear_large` — הראשון מעולם לא יוצר כתמונה שקופה אמיתית, השני חזר פגום/לא-תואם למפרט וטרם נוצר מחדש). ל-4 רכיבים (`switch_plate`/`springboard`/`cutter_shears`/`punch_arm`) יש גם פריים idle וגם action; ל-`charge_barrel` יש גם פריים-פיצוץ נפרד; ל-`fuse_cord` יש 3 פריימי הבהוב; ל-`crate_wood` יש גם גרסה שבורה — **אף אחד מהפריימים הנוספים האלה עדיין לא מחובר בפועל** (רק הפריים הבסיסי/idle של כל רכיב פעיל כרגע) — זה השלב הבא, לא בוצע בסשן הזה.

**שילוב בקוד (`render.rs`):** `static_texture_for(part_type) -> Option<&str>` ממפה סוג-רכיב לנתיב תחת `assets/` (רק ל-27 שיש להם אמנות). `decorate()` נבדק מול המיפוי הזה קודם ומחזיר מיד אם יש אמנות אמיתית — כדי לא לצייר גם את הדקורציה הפרוצדורלית *וגם* את התמונה האמיתית (כפילות ויזואלית). ה-swap בפועל קורה ב-`PartArtPlugin` חדש (`apply_real_art_system`) — **לא** ב-`body_factory::spawn_part` עצמו וגם **לא** נוסף ל-`PartsPlugin`/שום פלאגין ש-146 הטסטים ההדסלס משתמשים בו. הסיבה: `asset_server.load(path)` דורש את שרשרת `AssetPlugin`+`ImagePlugin`+`RenderAssetPlugin<GpuImage>` המלאה, וזו האחרונה בפועל דורשת RenderApp אמיתי — משהו ש-`MinimalPlugins` (הבסיס לכל הטסטים ההדסלס בפרויקט, בדיוק כדי להימנע מתלות ברינדור אמיתי) לא מספק, ולכן הייתה שוברת את כל 9 קבצי הטסט אם spawn_part עצמו היה תלוי ב-AssetServer. הפתרון: component חדש `PartType(String)` (ב-`body_factory.rs`, מוכנס על כל ישות שנוצרת ב-`spawn_part`, לא מזיק/לא נבדק ע"י שום טסט קיים) + system חדש שרץ *רק* ב-`Update` הרגיל ומחפש `Added<PartType>`, טוען את התמונה האמיתית (אם קיימת), ומחליף את ה-`Mesh2d`/`Sprite` הפרוצדורלי בה — נרשם רק דרך `app.rs`'s ה-`App::new()` האמיתי (עם `DefaultPlugins`), אף טסט לא מוסיף אותו. טסט drift-guard חדש (`render.rs`'s `every_static_texture_path_exists_on_disk`) מוודא שכל נתיב שה-mapping מצביע עליו קיים בפועל על הדיסק — כדי לתפוס טעות-הקלדה בנתיב לפני שהיא נכשלת בשקט ב-runtime (בדיוק כמו באג הגופנים של M9 למעלה). **147 טסטים עוברים** (146 + הטסט החדש), ואומת חזותית מול הבינארי האמיתי תחת Xvfb: `ball_wood` (ball-shape, מוכיח את מסלול הסרת ה-`Mesh2d`) ו-`plank_wood`/`bin_target` (box-shape) כולם מוצגים נכון בגודל הנכון בלי שאריות פלייסהולדר, ב-~60fps יציב, ללא אזהרות טעינת-נכסים ביומן.

**המשך: אנימציית idle/action/הבהוב מחוברת בפועל (לא רק frame בסיסי).** ארבעת הרכיבים עם זוג idle/action (`switch_plate`/`springboard`/`cutter_shears`/`punch_arm`), ה-explosion-overlay של `charge_barrel`, וה-3-frame flicker של `fuse_cord` — כולם היו רק "frame בסיס סטטי" בעדכון הקודם. עכשיו כל אחד מחובר למצב-משחק אמיתי:

- **`switch_plate`**: `sync_switch_plate_sprite` (ב-`render.rs`) קורא ישירות את אותו `SwitchPlate.is_on` שה-energy graph כבר קובע — אין דרך שהאנימציה "תשקר" לגבי המצב החשמלי בפועל.
- **`cutter_shears`**: קורא `CutterShears.triggered` (דגל one-shot קיים) — נשאר ב-action-frame לצמיתות אחרי ההפעלה הראשונה, תואם להתנהגות האמיתית (חותך פועל פעם אחת).
- **`punch_arm`**: קורא `PunchArm.was_touching` (השדה שהמערכת הפיזית עצמה כבר משתמשת בו) — arm מוארך בדיוק כל עוד יש מגע אמיתי, לא flash קבוע.
- **`springboard`**: **דרש component חדש** (`parts/springboard.rs`, מודול חדש) — springboard הוא גוף פיזי טהור (restitution גבוה בלבד), בלי שום component התנהגות קודם, ולכן אין אות קיים לקרוא ממנו. נוסף `Springboard` marker + `SpringboardFlash` (headless-safe, tick-counter כמו `FuseFlicker`) שמופעל ע"י `CollisionEvent::Started` אמיתי ומתפוגג לבד אחרי ~0.15 שנ' — קוסמטי בלבד, לא נוגע בפיזיקת הקפיצה עצמה (Rapier's restitution).
- **`charge_barrel`**: הבעיה כאן הפוכה — הברזל **מתפוצץ ונעלם** (`despawn`) באותו טיק שהוא מתפוצץ (`atmosphere.rs`), אז אין "ישות שורדת" להציג עליה frame-פיצוץ. נוסף `ExplosionEffect` — ישות חדשה, headless-safe, שנוצרת במקום הברזל *לפני* ה-despawn שלו, עם tick-counter קצר (~0.25 שנ') שמנקה את עצמה לבד; `render.rs` שם עליה את התמונה האמיתית ברגע שהיא נוצרת.
- **`fuse_cord`**: נוסף `FuseFlicker` (headless-safe, מונה frame 0-2 + tick-counter, מתקדם רק כש-`is_burning`) — **אבל בפועל לא מוצג!** ראה הבאג למטה.

**באג אמיתי שנמצא ותוקן (לא רק "עדיין לא מחובר" — ממש לא-נראה בפועל):** אימות חזותי תחת Xvfb (בדיוק אותה בדיקה קפדנית כמו למעלה) גילה של-`fuse_cord` אין שום ייצוג חזותי על המסך — לא ריבוע צבע שטוח, לא כלום, רווח ריק לגמרי בין הנר לחבית. נבדק ואומת ב-3 שכבות: (1) טסט הדסלס חדש הוכיח שה-`Sprite` הבסיסי כן קיים על הישות, עם `custom_size`/`transform`/`color` תקינים לחלוטין (`Vec2(100,6)`, צבע כחול אטום) — כלומר ה-ECS state תקין ב-100%; (2) שני טסטים הדסלס חדשים נוספים הוכיחו שהלוגיקה של ההצתה/בעירה (`FuseFlicker` מתקדם, `ExplosionEffect` נוצר ונעלם) עובדת נכון; (3) השורש בפועל: אמנות ה-`fuse_cord` (מ-DALL·E) צוירה כקו-פיתול אלכסוני על קנבס כמעט-מרובע (יחס ~1.3:1), בעוד ה-collider בפועל הוא פס דק 100×6 (יחס ~16.7:1) — מתיחת `custom_size` הכפויה למידות ה-collider המדויקות דוחסת את האמנות פי ~13 אנכית, מעבר לסף שבו הקווים הדקים שורדים בכלל (מאומת בהשוואה: `conveyor`/`plank_wood`/`beam_steel`/`lever_seesaw` — כולם גם "מוארכים", אך רק ביחס-פער מתון של 1.6–2.2x על צורה מלאה-גוון, ואלה כן נראים תקין בפועל, כמו ש-`plank_wood` כבר הוכיח למעלה). **תוקן** ע"י הוצאת `"fuse_cord"` מ-`static_texture_for`'s ה-mapping לגמרי (חוזר ל-placeholder הפרוצדורלי המקורי, שכן נראה תקין) — אבל מודגש: זה תיקון-נראות זמני, לא תיקון-אמנות; יידרש generation חדש של אמנות ל-`fuse_cord` בפרופורציה נכונה (קו ישר אופקי דק, לא פיתול אלכסוני) לפני שאפשר להחזיר אותו למיפוי. `sync_fuse_cord_sprite` נשאר בקוד עם guard זהה (מחזיר מיד אם `static_texture_for("fuse_cord")` הוא `None`) כדי שההחזרה תהיה שינוי חד-שורתי כשהאמנות הנכונה תהיה קיימת — `FuseFlicker` עצמו ממשיך לפעול ונבדק, רק לא מוצג.

**טסטים חדשים (`tests/part_art_animation_state.rs`):** 3 טסטים חדשים, כולם משתמשים בפיקסצ'רים אמיתיים קיימים (לא הומצאו) — `lvl_a08_fuse` (שרשרת נר→פתיל→חבית) ו-`lvl_a02_bounce` (springboard): מוכיחים ש-fuse עובר בין יותר מ-flicker-frame אחד תוך כדי בעירה אמיתית, ש-`ExplosionEffect` נוצר תוך 1200 טיקים מהפעלת רמת-הברזל האמיתית ואז מנקה את עצמו תוך 200 טיקים נוספים, וש-קפיצה אמיתית של כדור על springboard מדליקה את ה-flash ואז מנקה אותו תוך 60 טיקים. **150 טסטים עוברים בסה"כ.** אומת חזותית בנוסף: `switch_plate` נראה נכון (idle, לוח כחול עם כפתור מוגבה) ברמת `lvl_c04_just_in_time`, וכדור-ברזל אמיתי שנופל עליו בפועל (לא רק בטסט) מכסה אותו במדויק — מוכיח שהמנגנון הפיזי-חשמלי שה-sprite-sync קורא ממנו אכן מגיב למגע אמיתי.

השורה הקודמת ("מצב חופשי, שמירה, שיתוף שלבים") הייתה שורה אחת בלי שום החלטת-עיצוב אמיתית מאחוריה — אי אפשר להעריך זמן-פיתוח לדבר שלא הוגדר. זו הגדרה בפועל, מבוססת על מה שכבר קיים בקוד (לא ניחוש):

**תלות-קדם אמיתית, לא רק "יהיה נחמד":** M5's סיכום כבר קבע במפורש שסיבוב/היפוך/מחיקה/Inspector (§3.4) וכלי-חיבור חבל/מסוע/חוט (§3.5) נדחו ל-"M6+", ו-M6's סיכום מאשר ש-Inspector ו-LevelSelect **טרם מומשו** בפועל. Sandbox בלי היכולת למחוק רכיב, לסובב אותו, או לחבר `switch_plate` ל-`motor_electric` הוא כמעט חסר תועלת — אלה לא "נחמד להיות", הם חלק מהיקף M9 בפועל (או שצריך להיסגר כ-M6.5/M6.9 נפרד ממש לפני זה).

**"מצב חופשי":** מסך Sandbox חדש נכנס ל-`GameState::Edit` עם `EditorState` שה-`LevelFile` שלו ריק (רק `ground`, בלי `winConditions`/`failConditions`, בלי מגבלת זמן) — אותו state machine בדיוק כמו שלב רגיל, לא מסלול-קוד נפרד. ה-`partsBin` שלו כולל **כל** `partType` מ-`data/parts/*.json` (לא רשימה נעולה כמו בשלבי-הסיפור) בכמות בלתי-מוגבלת — הדרך הפשוטה ביותר: דגל `unlimited: true` חדש ב-`PartsBinEntry` (`level_file_format.rs`) שה-UI מכבד במקום לספור למטה מ-`count`.

**שמירה — פורמט קיים, לא חדש:** `LevelFile`/`PlacedPart`/`Connection` וכו' ב-`level_file_format.rs` הם `Deserialize`-בלבד כרגע (טעינה בלבד; אין עדיין שום מנגנון שכותב JSON). M9 מוסיף `Serialize` לאותם structs בדיוק, ומערכת ש"קוראת" את מצב ה-ECS החי (`PlacedId`/`Transform`/`PartTags`/`RopeConnection` הקיימים) בחזרה ל-`LevelFile` יחיד — **אותו schema המדויק** שכל 60 השלבים הרשמיים כבר משתמשים בו, לא פורמט נפרד. נשמר לתיקיית `saves/` בנתיב app-data של הפלטפורמה (לא `levels/` — זו built-in דרך `include_str!` בזמן קומפייל, לא נסרקת ב-runtime).

**טעינה / "My Levels":** מרחיב את מסך LevelSelect (עדיין לא קיים — M6 debt) בטאב שני שסורק את `saves/` ב-runtime (לא `level_catalog.rs`'s `ALL_LEVELS`, שקבוע בזמן קומפייל) ומציג את השלבים השמורים לצד 60 הרשמיים.

**שיתוף — אין ולא מתוכנן backend בפרויקט הזה (אפס שרת, אפס auth, נבדק בקוד).** "שיתוף" = ייצוא אותו קובץ JSON השמור: באנדרואיד דרך Android Share Sheet הסטנדרטי (`Intent.ACTION_SEND`), בדסקטופ פתיחת תיקיית ה-saves/שמירה-בשם. "ייבוא" = בחירת קובץ `.json` שהתקבל, עובר דרך **בדיוק** אותו `serde_json::from_str::<LevelFile>` והטוען שכל שלב רשמי כבר עובר — קובץ פגום נכשל באותו מסלול שגיאה ידידותי הקיים, לא בקריסה.

**מפורשות מחוץ לתחום ל-M9 v1:** כל שרת/חשבון/cloud sync/leaderboard מקוון — אלה דורשים backend שלא קיים ולא מתוכנן. רעיון אפשרי אחרי launch, לא בהיקף הנוכחי.

**קריטריון קבלה בפועל:** שחקן בונה שלב מאפס ב-Sandbox עם כל סוגי ה-parts (כולל חיבור חבל/מסוע/חוט), שומר אותו לקובץ, סוגר וטוען מחדש את המשחק, פותח את אותו שלב שמור מ-"My Levels" ורואה את אותה מפה בדיוק (round-trip מאומת). מייצא את הקובץ, חבר מייבא אותו על מכשיר אחר ורואה בדיוק את אותה מפה.
...
### 7.8 צ'קליסט שחרור (M10/M11) — מה קוד יכול לעשות מול מה שרק המפתח יכול

כל 60 שלבי M8 הושלמו; זה מעביר את נקודת-החסם הבאה ל-M10/M11, שרובו **לא ניתן לביצוע מתוך הסביבה הזו** — לא חוסר-יכולת טכני, אלא כי הוא דורש דברים שרק למפתח יש: המכשיר הפיזי שלו (M3.5, עדיין פתוח), חשבונות (Google Play Console $25, itch.io), ומפתחות חתימה (keystore) שאסור שייווצרו/יישמרו בסביבת CI משותפת בלי הסכמה מפורשת.

**מה שכבר בוצע בקוד (לא צריך יותר עבודה):**
- CI דסקטופ ירוק (`desktop.yml`, שלוש המערכות).
- CI אנדרואיד ירוק שמייצר APK **debug** אמיתי (`android.yml`, ארטיפקט `chainworks-debug-apk`).
- `[package.metadata.android]` ב-`Cargo.toml` מוגדר עם `target_sdk_version`/`min_sdk_version` תקינים.
- **`desktop.yml` מעלה כעת ארטיפקט "הורדה והרצה" אמיתי** לכל אחת משלוש המערכות (`dist/`: הבינארי + `assets/` לצידו) — לא רק build+test כמו קודם. תוקן ישירות מבאג אמיתי שנתפס: `bevy_asset` לא מוצא גופנים כשמריצים בינארי מופץ ישירות (ללא `CARGO_MANIFEST_DIR`, שקיים רק כש-Cargo עצמו מריץ את הפרוצס) — מאומת מקומית תחת Xvfb, לפני ואחרי.

**חסימה קשה שרק המפתח יכול לפתור — לא עוקפים אותה:**
1. **שם החבילה (`package = "com.chainworksgame.app"` ב-`Cargo.toml`) הוא placeholder.** ההערה בקוד כבר אומרת את זה: "MUST change to your own reverse-DNS before any real store submission" — **ולא ניתן לשינוי אחרי פרסום ראשון ל-Play**. זו החלטה חד-כיוונית שרק המפתח יכול לקבל (תלוי בדומיין/שם-ארגון שלו ב-Play Console) — ראתה שאלה נפרדת על זה.
2. **בדיקת מכשיר פיזי (M3.5)** — עדיין לא בוצעה. אין דרך לוודא מתוך הסביבה הזו שה-APK בכלל עולה על טלפון אמיתי.
3. **Keystore לחתימת release** — חייב שהמפתח ייצור אותו (`keytool -genkey`) ויעלה אותו + הסיסמאות שלו כ-GitHub Secrets בעצמו. אין תרחיש שבו סוכן-קוד יוצר/מחזיק מפתח חתימה קבוע בשם המפתח.
4. **חשבון Google Play Console ($25 חד-פעמי) / itch.io** — הרשמה בפועל, לא קוד.

**מה שכן אפשר להכין עכשיו בקוד, מראש, בלי לחכות למכשיר/לחשבון:** job חדש ב-`android.yml` שבונה APK **release** חתום, אבל **מותנה בקיום ה-Secrets** — כשאין `ANDROID_KEYSTORE_BASE64` מוגדר, ה-job הזה פשוט מדלג (`skipped`), ולא שובר את ה-CI הקיים/הירוק. ברגע שהמפתח יוצר keystore ומוסיף את ה-Secrets, אותו job מתחיל לרוץ בלי שינוי קוד נוסף. ראה `.github/workflows/android.yml`'s `android-release-apk` job.

**עדכון: שני פריטים נוספים שניתן להכין בלי לחכות למכשיר/לחשבון — בוצעו.**

1. **`.github/workflows/release.yml` (חדש) — נתיב GitHub Releases לדסקטופ.** M11's קריטריון עצמו אומר "דסקטופ: itch.io **או** GitHub Releases" — הראשון דורש חשבון, השני לא (אותו repo, אותה תשתית CI). ה-workflow משתמש שוב באותם שלבי build+package מאומתים מ-`desktop.yml` (אותו `dist/`: בינארי + `assets/` לצידו, אותו תיקון ל-`bevy_asset`/`CARGO_MANIFEST_DIR`) לכל שלוש המערכות, מכווץ כל אחד ל-zip, ומצרף את שלושתם ל-GitHub Release אמיתי — רק על push של tag בתבנית `v*` (למשל `v0.1.0`). ה-Release נוצר כ-**draft** (לא מפורסם אוטומטית) — כי ה-workflow רץ ללא השגחה בכל push של tag, וזה משמר בדיקה אנושית (לחיצה על "Publish") לפני שמשהו הופך גלוי לשחקנים בפועל, לא רק push של tag בעצמו. **לא ניתן לאמת end-to-end מהסביבה הזו:** `workflow_dispatch` (מריץ רק את job ה-build/zip, לא את הפרסום) נכשל ב-404 — ה-API של GitHub לדיווש-לפי-שם-קובץ פותר workflows רק מול ה-branch שמוגדר כ-default של ה-repo (שאינו ה-branch הזה — ראה ההערה למעלה על שני ה-branch-ים הבלתי-קשורים), ופתיחת tag `v*` אמיתי כדי לבדוק את ה-publish job עצמו הייתה יוצרת GitHub Release אמיתי (אפילו כ-draft) בלי אישור מפורש מהמפתח, שזו פעולה גלויה/קשה-להחזרה שלא נכון לבצע רק כדי לבדוק YAML. אומת ידנית: YAML תקין (`python3 -c "import yaml; yaml.safe_load(...)"`), ומשתמש שוב באותם שלבים בדיוק ש-`desktop.yml` כבר מאומת שעובדים.
2. **`docs/store-listing.md` (חדש) — תוכן מוכן-להדבקה לעמוד itch.io/Play Console.** תיאור קצר/מלא, feature bullets, תשובות Data Safety Form, ותשובות שאלון-דירוג-תוכן — כולן נגזרות מהתנהגות הקוד בפועל, לא ניחוש: `grep` על כל הריפו מאמת שאין קוד רשת/analytics/telemetry בכלל (אז "לא נאסף מידע" ב-Data Safety הוא עובדה, לא הנחה), ו-`[package.metadata.android]` לא מכריז שום permission (אז אין הצדקה לשאלה "האם המשחק ניגש לאינטרנט/מיקום/וכו'" מלבד "לא"). כל התוכן נבדק במפורש מול טבלת §4's מותר/אסור — אין "TIM"/"Incredible Machine"/"Rube Goldberg" (סימן מסחר רשום) בשום מקום, ואין השוואה מפורשת ("like TIM") שהיא בדיוק המלכודת ש-§4.2 מזהירה מפניה. **מה שבמפורש לא הוחלט בתוכן הזה:** תמחור, screenshots/trailer אמיתיים (עדיין לא רלוונטיים כל עוד §3.9's עבודת ה-visual-polish לא הגיעה למצב שמשקף מה ששחקן באמת יראה), ושם-החבילה (עדיין placeholder, כמו למעלה).
