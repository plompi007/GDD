...
| **M8** | **תוכן** | כל 60 שלבים + P0/P1 מלאים + כל `solutions[]` golden tests עוברים ב-CI | **⏳ חלקי: 9/10 משלבי חטיבה A ממומשים ומאומתים (`tests/content_levels.rs` + `tests/gear_and_pulley.rs`), כולל שלוש תת-מערכות חדשות: A01 (נפילה חופשית), A02 (`springboard`), A04 (`lever_seesaw`), **A05 (`atmosphere.rs`'s `FieldSystem` חדש — שדה PNEUMATIC מלבני מ-`fan_blower`, דוחף כל גוף דינמי לפי `windFactor`/מסה הפוכה)**, **A06 (`switch_plate` חדש — IMPACT דרך `CollidingEntities` קיים, `SimSet::CollisionRouting`, ELECTRIC out ל-`EnergyGraph`)**, A07 (גלגלי שיניים+מסוע), **A08 (`atmosphere.rs`'s `ThermalSystem` חדש — `candle`/`fuse_cord`/`charge_barrel`: הצתה מבוססת-קרבה, התקדמות בעירה קבועה, פיצוץ רדיאלי + הריסת `DESTRUCTIBLE`)**, **A09 (`sim/collision_router.rs` חדש — כלל תגית גנרי SHARP↔POPPABLE, ו-`cutter_shears` חדש שחותך `RopeConnection` וקרבה)**, **A10 (`punch_arm` חדש — אימפולס קדימה על מגע חדש דרך אותו `CollisionRouting`)**. A03 קיים כשלב-פיתוח (`lvl_a03_pulley_lift`, לא לפי המספור המקורי). באג אמיתי שנמצא ותוקן באגב: `PartTags` (`level_load.rs`) כלל בעבר רק תגיות per-instance מה-level JSON, לא את תגיות ה-part-type המובנות (`DESTRUCTIBLE`/`FLAMMABLE`/`POPPABLE`/`SHARP` מ-`data/parts/*.json`) — כלל SHARP↔POPPABLE ופיצוץ-הורס-DESTRUCTIBLE היו no-op שקט לפני התיקון. חטיבת B (B_COMBOS, שלבים 13–28 לפי §5.8) התחילה: 5/16 שלבים ממומשים ומאומתים (`tests/content_levels_b.rs`): B01 "Cut Loose" (חיתוך→נפילה→לחיצת מתג→גלגלי שיניים+מסוע — 3 מכניקות בשרשרת אחת), B02 "Relay" (שני פתילים ברצף, הבערה מפתיל לפתיל — "מה קורה כשלהבה נוגעת בפתיל" מסעיף 1.4.3, לא נבדק קודם), B03 "Break Through" (כלל תגית FRAGILE חדש ב-`sim/collision_router.rs` — פגיעה במהירות מעל 18 מ'/ש' מנפצת; היה מתועד ב-§1.4.2 אבל מעולם לא מומש. גם `punch_arm.impulsePower` הפך לניתן-לדריסה per-instance דרך `params.power`), B04 "Fling the Trigger" (מנוף (A04) משגר כדור אל לוח לחיצה (A06) שמפעיל גלגלי שיניים+מסוע (A07) עבור כדור-נושא נפרד — שרשרת של 3 מכניקות דרך `WIRE`/`BELT` בלבד, בלי קוד חדש), B05 "Windswept" (רוח (A05) דוחפת כדור זכוכית רגיל — לא בלון — אל לוח לחיצה; הבדיקה הראשונה של `windFactor`/מסה כברירת מחדל על גוף לא-בלון). **חטיבת C (C_TIMING, שלבים 29–44) התחילה: 1/16 שלבים — C01 "Slow Fuse" (כלל THERMAL×TENSION חדש ב-`atmosphere.rs`/`rope_network.rs`: חבל בטווח מקור חום נשרף ונחתך, בדיוק כמו §1.4.1 המתועד אך מעולם לא מומש. גם באג פיזיקה אמיתי שנמצא תוך כדי: כדור שנופל אנכית לחלוטין ונוחת על קרקע שטוחה עם מהירות אופקית אפס יכול לפגוע בכיוון-חיכוך ניוון (degenerate) בפותר הפיזיקה ולקבל בעיטה אופקית בלתי-צפויה — A09 נחת נקי במקרה, C01 לא; תוקן ע"י מלכודת-תפיסה (קירות קטנים משני צדי הדלי) שלא תלויה במזל הנחיתה**. חטיבת D (עוד 16 שלבים) — לא התחילה** |
