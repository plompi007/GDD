...
- **קידוד-צבע לפי אנרגיה על `WIRE`:** לחיבורי `WIRE` (ELECTRIC) אין ישות פיזית מתמשכת בכלל (בניגוד ל-`RopeConnection`/`BeltConnection`) — `connect_tool_system`/`spawn_connections` רק קוראים ל-`EnergyGraph.connect()` ולא שומרים entity. ציור קו עבורם ידרוש state חדש (איפה `switch_plate` X מחובר ל-`motor_electric` Y), לא רק system חדש. לא מומש.
