...
| **M8** | **תוכן** | כל 60 שלבים + P0/P1 מלאים + כל `solutions[]` golden tests עוברים ב-CI | **⏳ חלקי: 4/10 משלבי חטיבה A ממומשים ומאומתים (`tests/content_levels.rs` + `tests/gear_and_pulley.rs`): A01 (נפילה חופשית), A02 (`springboard` חדש — קפיצה מרוצפת עם קיר-בור לתפיסה), A04 (`lever_seesaw` חדש — `RevoluteJoint` סביב עוגן קבוע, "ממיר נפילה לזריקה" באמת), A07 (גלגלי שיניים+מסוע). A03 קיים כשלב-פיתוח (`lvl_a03_pulley_lift`, לא לפי המספור המקורי בתוכן). A05/A06/A08/A09/A10 דורשים תת-מערכות חדשות לגמרי (שדה רוח, ניתוב IMPACT ממגע, מערכת תרמית/פתיל, חיתוך) — לא מומשו. חטיבות B/C/D (50 שלבים נוספים) — לא התחילו** |
