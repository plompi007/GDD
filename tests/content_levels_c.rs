...
#[test]
fn lvl_c09_punch_the_flame_is_solvable() {
    let json = include_str!("../levels/C/lvl_c09_punch_the_flame.json");
    let mut app = build_app(json);
    assert!(
        run_until_solved(&mut app, MAX_TICKS_TO_SOLVE),
        "lvl_c09_punch_the_flame did not reach Solved within {MAX_TICKS_TO_SOLVE} ticks"
    );
}

#[test]
fn lvl_c09_punch_the_flame_is_deterministic() {
    let json = include_str!("../levels/C/lvl_c09_punch_the_flame.json");
    let first = subject_final_position(json, 790);
    let second = subject_final_position(json, 790);
    let third = subject_final_position(json, 790);
    assert_eq!(first, second, "run 1 and run 2 diverged");
    assert_eq!(second, third, "run 2 and run 3 diverged");
}

#[test]
fn lvl_c10_no_time_to_spare_is_solvable() {
    let json = include_str!("../levels/C/lvl_c10_no_time_to_spare.json");
    let mut app = build_app(json);
    assert!(
        run_until_solved(&mut app, MAX_TICKS_TO_SOLVE),
        "lvl_c10_no_time_to_spare did not reach Solved within {MAX_TICKS_TO_SOLVE} ticks"
    );
}

#[test]
fn lvl_c10_no_time_to_spare_is_deterministic() {
    let json = include_str!("../levels/C/lvl_c10_no_time_to_spare.json");
    let first = subject_final_position(json, 875);
    let second = subject_final_position(json, 875);
    let third = subject_final_position(json, 875);
    assert_eq!(first, second, "run 1 and run 2 diverged");
    assert_eq!(second, third, "run 2 and run 3 diverged");
}

#[test]
fn lvl_c11_two_ways_to_cut_is_solvable() {
    let json = include_str!("../levels/C/lvl_c11_two_ways_to_cut.json");
    let mut app = build_app(json);
    assert!(
        run_until_solved(&mut app, MAX_TICKS_TO_SOLVE),
        "lvl_c11_two_ways_to_cut did not reach Solved within {MAX_TICKS_TO_SOLVE} ticks"
    );
}

#[test]
fn lvl_c11_two_ways_to_cut_is_deterministic() {
    let json = include_str!("../levels/C/lvl_c11_two_ways_to_cut.json");
    let first = subject_final_position(json, 455);
    let second = subject_final_position(json, 455);
    let third = subject_final_position(json, 455);
    assert_eq!(first, second, "run 1 and run 2 diverged");
    assert_eq!(second, third, "run 2 and run 3 diverged");
}

#[test]
fn lvl_c12_wind_up_is_solvable() {
    let json = include_str!("../levels/C/lvl_c12_wind_up.json");
    let mut app = build_app(json);
    assert!(
        run_until_solved(&mut app, MAX_TICKS_TO_SOLVE),
        "lvl_c12_wind_up did not reach Solved within {MAX_TICKS_TO_SOLVE} ticks"
    );
}

#[test]
fn lvl_c12_wind_up_is_deterministic() {
    let json = include_str!("../levels/C/lvl_c12_wind_up.json");
    let first = subject_final_position(json, 835);
    let second = subject_final_position(json, 835);
    let third = subject_final_position(json, 835);
    assert_eq!(first, second, "run 1 and run 2 diverged");
    assert_eq!(second, third, "run 2 and run 3 diverged");
}

#[test]
fn lvl_c13_hurry_gently_is_solvable() {
    let json = include_str!("../levels/C/lvl_c13_hurry_gently.json");
    let mut app = build_app(json);
    assert!(
        run_until_solved(&mut app, MAX_TICKS_TO_SOLVE),
        "lvl_c13_hurry_gently did not reach Solved within {MAX_TICKS_TO_SOLVE} ticks"
    );
}

#[test]
fn lvl_c13_hurry_gently_is_deterministic() {
    let json = include_str!("../levels/C/lvl_c13_hurry_gently.json");
    let first = subject_final_position(json, 700);
    let second = subject_final_position(json, 700);
    let third = subject_final_position(json, 700);
...
#[test]
fn lvl_c14_slow_release_is_solvable() {
    let json = include_str!("../levels/C/lvl_c14_slow_release.json");
    let mut app = build_app(json);
    assert!(
        run_until_solved(&mut app, MAX_TICKS_TO_SOLVE),
        "lvl_c14_slow_release did not reach Solved within {MAX_TICKS_TO_SOLVE} ticks"
    );
}

#[test]
fn lvl_c14_slow_release_is_deterministic() {
    let json = include_str!("../levels/C/lvl_c14_slow_release.json");
    let first = subject_final_position(json, 450);
    let second = subject_final_position(json, 450);
    let third = subject_final_position(json, 450);
...
#[test]
fn lvl_c16_one_careful_run_is_solvable() {
    let json = include_str!("../levels/C/lvl_c16_one_careful_run.json");
    let mut app = build_app(json);
    assert!(
        run_until_solved(&mut app, MAX_TICKS_TO_SOLVE),
        "lvl_c16_one_careful_run did not reach Solved within {MAX_TICKS_TO_SOLVE} ticks"
    );
}

#[test]
fn lvl_c16_one_careful_run_is_deterministic() {
    let json = include_str!("../levels/C/lvl_c16_one_careful_run.json");
