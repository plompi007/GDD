...
registerTest('lvl_b01_bounce_combo: solved (bridge the gap, ride the gear-driven conveyor into the goal)', () => {
  const registry = loadPartRegistry();
  const level = loadLevelById('lvl_b01_bounce_combo');
  ...
  if (!result.solved) {
    throw new Error(`expected SOLVED with the bridge placed, got failed=${result.failed} reason=${result.failReason} ticks=${result.ticksRun}`);
  }
});
